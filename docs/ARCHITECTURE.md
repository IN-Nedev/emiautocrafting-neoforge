...
Lectern and AE2 CRAFT operations send a bounded number of native single-craft actions, accumulate one output stack on the cursor and deposit it once into an initially empty player slot. `CraftBatches` limits execution by the outstanding request, stack capacity, reserved ingredient budget, refillable stock and the smallest matching grid stack for each ingredient. AE2 cannot refill from loose player ingredients. This avoids unbounded native shift-crafting and does not assume that uneven grid stacks can rebalance themselves. Recipes with remainders, single-step mode and Crafting Station stay at one execution. Confirmation verifies the aggregate consumption and output; partial/rejected batches are never retried automatically.
